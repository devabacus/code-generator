...
4. **ВНИМАНИЕ (изменилось в TASK-042, уточнено в TASK-043):** ручной `git diff` перед regen больше не нужен как страховка — preflight сам сравнивает файл на диске с ledger'ом (`<project>/.codegen/ledger.json`) и при расхождении **останавливает генерацию до первой записи** с non-zero exit и diff'ом. Что это значит на практике: правка внутри `:base` (handleSyncEvent, createX, mappers) и правка любого generated-файла дают **conflict**, а не молчаливую потерю. Осознанная перезапись — **по файлу**:
   - `--overwrite-existing <путь1>,<путь2>` (флаг повторяем, пути — project-relative, ровно как в отчёте о конфликте) — перезаписывает **только перечисленное**. Остальные конфликты остаются нетронутыми, **baseline им не сеется** — на следующем прогоне они снова конфликтуют. Это не баг, это и есть защита.
   - голый `--overwrite-existing` без списка — историческое «перезаписать все конфликты», с явным предупреждением в выводе.
   - **смешивать формы нельзя:** `--overwrite-existing a.dart --overwrite-existing` (голый флаг после списка — реалистично для скрипта-обёртки) раньше молча схлопывался commander'ом в «перезаписать ВСЁ». Теперь это **отказ до первой записи**; оставь одну форму.
   - опечатка в пути (нет среди конфликтов) → **fail-fast** с перечислением доступных, ни одной записи. Повтор той же команды после успешной перезаписи тоже упрётся в fail-fast: перезаписанные файлы больше не конфликтуют.
   - перед каждой деструктивной записью прежнее содержимое копируется в `<project>/.codegen/backup/<timestamp>/<тот же относительный путь>`, путь печатается в выводе. Каталог скрыт от git вложенным `.codegen/.gitignore` (`backup/`); сам `ledger.json` остаётся под контролем git.
   - **Файл, оставленный «как есть», не трогает НИКТО** (TASK-043 R2): ни фаза apply, ни `relation_patcher` / `orchestrator_patcher` / `app_database_generator` — они пишут в обход plan/apply и получают гард `PreservedFiles`. Плата: машинный регион в таком файле остаётся от прошлой генерации (может быть устаревшим) — генератор печатает об этом отдельным блоком, а не умалчивает.

   Не покрыто guard'ом (**[BUG-030](ai/project/bug-reports/030-relation-patcher-otm-region-outside-guard.md), открыт**): правка внутри `:oneToManyMethods` **сама по себе не порождает конфликт** — baseline merge-файла хеширует только `:base`, поэтому при штатном regen такая правка по-прежнему затирается патчером молча. TASK-043 закрыл только соседний случай: если файл попал в конфликт по другой причине и ты выбрал его сохранить — патчер его больше не тронет. `:syncImports` / `:syncEntityTypes` / `:syncRegistrations` в orchestrator — патчатся идемпотентно (existing entries сохраняются + новый append'ится).
