...
**Status:** Draft (Phase A Sub-A2, awaiting Sub-A5 multi-agent review + User counter-sign в Sub-A6)
**Date:** 2026-05-03
...
Per [sync-core-audit.md](sync-core-audit.md) (Sub-A3 ✅ complete 2026-05-03):
...
**Rationale (full evidence в [backend-strategy-rationale.md](backend-strategy-rationale.md) + [weight-v1-schema-audit.md](weight-v1-schema-audit.md)):**
...
Following ambiguities flagged for multi-agent review (architecture / sync / test reviewers) в Sub-A5:

1. **OQ-1 (Section 3.1 — generate Riverpod data providers):** Включает ли generation `loggerServiceProvider` injection в data providers (per ADR-0001 в codegen)? Анти-example 3.4.4 запрещает `AsyncNotifier с business logic`, но не упоминает logger wiring явно. Architecture reviewer — clarify expected behavior.
2. **OQ-2 (Section 3.4.2 — generate sync_core adapters):** Какой default `ConflictPolicy` emit (last-write-wins per sync_core 0.3.0 Reasonable default vs explicit unset requiring consumer choice)? Sync reviewer — verify против sync_core ADR-0001..0003 conflict policy contract.
3. **OQ-3 (Section 4.1 scope binding):** Default scope binding `customer_${currentUser.customerId}` assumes consumer имеет authenticated user context при template generation runtime. Что если simplified template generated для project без auth (e.g. local-only synthetic t<200>)? Sync reviewer — clarify scope default или mark "consumer responsibility, no default emitted".
4. **OQ-4 (Section 5 backend strategy):** Option 2 trigger criteria documented в backend-strategy-rationale.md. Должен ли ADR-0005 копировать full trigger list inline (Section 5) или достаточно reference на rationale.md? Architecture reviewer — preferred ADR self-containment level.
5. **OQ-5 (Section 7 TBD placeholders):** Q7=e REJECT — Phase B-D decisions emerge из prototyping. Но "manifest markers для simplified template" (Section 7.3) overlaps с RelationPatcher / OrchestratorPatcher applicability questions из test-inventory-audit.md (open questions #1-#2). Test reviewer — verify alignment между ADR Section 7.3 и test-inventory-audit.md open questions.
6. **OQ-6 (Section 3.4.3 anti-example syncStatus inline):** Этот anti-example описывает legacy weight v1 pattern (per Sub-A0.5 audit). Adversarial reviewer — есть ли non-weight scenario где `syncStatus` inline column оправдан (e.g. local-only entity без sync coverage)? Если да — добавить exclusion clause.
7. **OQ-7 (Section 4.3 Sub-A3 placeholder):** Sub-A3 audit ещё не started/completed. Sub-A6 finalize requires Sub-A3 deliverable. Если Sub-A3 surfaces HIGH risk → ADR draft возвращается на rework Section 4.3? Adversarial reviewer — verify acceptance criterion в TASK-021 task.md (Sub-A3 STOP-gate clause).

Sub-A5 reviewers expected resolve / clarify / defer эти OQs; resolutions integrated в Sub-A6 finalize pass.
...
- [TASK-021 task.md](task.md) — full Phase A scope + 7 sub-phases plan + STOP-gates
- [Sub-A0.5 weight v1 schema audit](weight-v1-schema-audit.md) — 4 Option 2 triggers evidence-based evaluation (0.5/4 active)
- [Sub-A1 backend strategy rationale](backend-strategy-rationale.md) — Option 1 confirmed + 4 trigger criteria + User sign-off recorded
- [Sub-A3 sync_core dual-running audit](sync-core-audit.md) — pending (Sub-A2 leaves placeholder в Section 4.3, Sub-A6 inserts recommendation после Sub-A3 deliverable)
- [Sub-A4 test inventory audit](test-inventory-audit.md) — (file, category, action) tuple table 18 файлов / 164 cases / 5 actions; 78% cases / 72% files universal evidence supporting Option 1
