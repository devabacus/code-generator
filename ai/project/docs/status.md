...
- под codegen **15 сущностей из 43**; на каждой **38 конфликтов** (`legacy-mismatch`, ledger
  отсутствует) → ≈570 файлов; **84% full-replace**;
...
- **риск локализован**: пересечение «ручные правки» × «зона перезаписи» на 4 сущностях —
  **6 файлов, все в `weighing`**; у `subscription`, `terminal_device`, `configuration` —
  пусто;
- **per-file preserve из TASK-043 не масштабируется** на 570 файлов — не хватает обратного
