...
## Постоянные разрешения агенту (решение владельца 2026-07-30)

Механические операции больше **не требуют спрашивать** — делай и показывай результат:

- **коммит в feature-ветку** (в `master` — никогда, только через PR);
- **создание PR** (`task.py pr` / `gh pr create`) — это не merge, откатывается закрытием,
  а гейт `report.md` не даст уехать шаблонной рыбе;
- **заведение bug-report'а и задачи** через `new_task.py` по находке в ходе работы;
- ранее выданные: `create-project` / `verify` / `generate-entity`.

**Требуют явного слова владельца:**

- **merge в master** — единственная необратимая операция («мержить»/«мерджи»);
- **push в репо шаблонов** (t115 / simplified);
- **содержательные развилки** — архитектурный выбор, которого нет в task.md/ADR; смена
  scope задачи; отказ от критерия приёмки. Block-rules из `AGENTS.md` в силе.

Обоснование (сессия 2026-07-30): за сессию было четыре остановки за словом, и все четыре
ответа — «да». Механические остановки не добавляли ценности, а незакоммиченная работа
дважды оказывалась в шаге от потери. Содержательная остановка (блок TASK-049) свою цену
отработала — она и остаётся.

⚠ **Автономнее агент от апдейта шаблона НЕ стал** — это отдельный вопрос, см. следующий
блок. Автономию задают правила владельца, а не схема задач.

---

## Апдейт AI-workflow шаблона (2026-07-30) — что поменялось в работе

Первый в истории проекта `sync --apply` (до этого мигрировали вручную). Прогнан извне,
проверен здесь: 16 изменённых core-файлов + 2 новых теста, 0 удалённых, `ai/project/**`
не тронут. `source_revision 0d8a59e → 654a017`.

Четыре вещи, меняющие повседневную работу:

- **`PYTHONIOENCODING=utf-8` для core-скриптов больше не нужен** — они сами ставят UTF-8 на
  stdout/stderr. До апдейта `sync.py --check` здесь **падал** `UnicodeEncodeError` на эмодзи
  (системный `python` — из ESP-IDF, cp1251), после — exit 0 без переменной. Для **своих**
  одноразовых скриптов переменная всё ещё нужна.
- **`task.py pr` отказывается брать тело PR из незаполненного `report.md`** (fail-closed,
  отказ ДО push и до коммита-перемещения — [task.py:320](../../../ai/core/scripts/task.py)).
  На 2026-07-30 гейт отказал бы на TASK-041/044/045/046 (шаблонная рыба), TASK-049 проходит.
  Это задумано: раньше PR уезжал с рыбой.
- **Ошибки линтов уходят в stderr**, а не только в stdout. Обвязки, парсящей их stdout, у нас
  нет — CI линты не вызывает.
- **`core/prompts/executor.prompt.md` переписан**, шаг «закоммить» убран (противоречил
  workflow). Наши проектные копии промптов при этом **прогнили v1-путями**: 33 ссылки на
  `ai/docs/`/`ai/scripts/` в `teamlead.prompt.md`, 10 в `executor.prompt.md` — пути, которых
  не существует. Апдейт core их не чинит и не должен.

**Поправки к прежнему отзыву (были неточны, подтверждено по коду):**

- поля `mode`/`max_attempts`/`verification_profile`/`checks` **читаются** — валидацией
  `task.py lint`, причём `mode: auto` переводит warning в error. Верная формулировка: нет
  потребителя, меняющего исполнение, а не «не читаются»;
- в инциденте с коммитом в master проглочен был **код возврата**, а не текст: `| tail` отдаёт
  статус `tail`. Внутри скрипта не чинится — только `set -o pipefail` на вызывающем. Правило
  «`task.py` вызывать отдельной командой, не в пайпе» остаётся в силе.

**Кириллические имена веток — почищено этим же апдейтом.** `run()` теперь задаёт
`encoding="utf-8"` при `capture=True`; до апдейта его не было, `current_branch()` декодировал
вывод git системной cp1251, и в `git push` уходил искажённый refspec →
`error: src refspec … does not match any`. Проверено на новом коде: `current_branch()` на
ветке с кириллицей возвращает имя точь-в-точь. `task.py pr` для наших задач (а у них русские
заголовки → русские ветки) снова рабочий.

⚠ **Урок, стоивший неверного вывода:** падение скрипта я сначала объявил «апдейтом не
закрытым» — но прогон шёл на ветке, отведённой **до** апдейта, то есть старым скриптом.
**Прежде чем объявлять дефект незакрытым, проверь, какая версия инструмента исполнялась.**
Ветка, созданная до апгрейда тулинга, несёт старый тулинг — это тот же класс, что правило
владельца «правка не даёт эффекта → сначала убедись, что исполняется именно правленый код».

**Тесты core запускаются напрямую, не через pytest** (`python ai/core/scripts/test_*.py`) —
это standalone-скрипты с `main()`; pytest в `requirements.txt` не заявлен и здесь не стоит.

**Не приехал** адаптер `.claude/` (3 субагента + 3 хука) — лежит вне `ai/`, sync его не
раздаёт. Отложен осознанно: субагентов надо сперва свести с
[model-policy.md](model-policy.md) (модель назначается явно, флагман запрещён).
