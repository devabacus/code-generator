...
- [x] `# codegen:junction: [a, b]` читается из комментария; **обе** формы (`[a,b]` и `true`) поддержаны
- [x] Маркер якорится на колонку 0 и читается ДО `yaml.load`; дубликат маркера → ошибка
- [x] Cross-validation: оба родителя существуют среди relation-полей и различны; malformed → fail-fast (не тихая деградация)
- [x] Migration-guard: настоящий YAML-ключ `junction` → внятная ошибка с инструкцией переноса
- [x] **serverpod generate PASS** на junction с директивой БЕЗ ручного вмешательства (доказать на свежем test-проекте: `verify` → serverpodGenerate errors=0)
- [x] Fallback-поведение НЕ изменено (тесты эвристики зелёные без правок логики)
- [x] Тест-фикстуры репо мигрированы на новый носитель
- [x] checks compile/lint/unit зелёные, baseline 322 не падает
