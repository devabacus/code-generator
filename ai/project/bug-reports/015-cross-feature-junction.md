...
**Статус:** CONFIRMED + PARTIAL FIX → **BLOCKED (архитектурный остаток)** (2026-07-21, TASK-039)
...
## Почему остаток BLOCKED (не чинился в TASK-039)

Оставшиеся 8 broken-импортов — в 5 подсистемах, **без** переиспользуемого механизма
резолюции (в отличие от table-слоя). Корректный фикс требует архитектурного решения,
...
- `junction: [a, b]` директива при cross-feature + serverpod generate — заблокирована
  Serverpod'ом (отдельный край, см. выше).
- Остаток BLOCKED (5 слоёв presentation/repository/providers/usecases) — **не
  исправлен**, значит cross-feature junction в целом **не работает** end-to-end.
