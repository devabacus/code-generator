...
**Mitigation/correct modeling (всё ещё рекомендовано):** one-to-many = child FK only (`childId: relation(parent=X)` + `X?, scope=serverOnly`); parent НЕ держит flutter back-relation list (паттерн t115 `task`→`category`). Теперь back-relation на parent безопасен (стрипается), но не несёт flutter-семантики. Junction back-relation (`<j>Maps: List<JMap>`) — также стрипается. **Non-blocking follow-ups:** pre-flight reject для `List<scalar>` на synced-entity; cleanup redundant `Map`-эвристики (mapUrl/bitmapData latent false-positive). См. [BUG-027](../bug-reports/027-one-to-many-back-relation-regular-entity-leaks-into-flutter-entity.md).
