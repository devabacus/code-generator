...
- **TASK-033** (suggested per TASK-031 Rev 3 discovery 2026-05-27): **t115 generate-entity disk write bug investigation** — generate-entity CLI на t115 path reports `files_modified` array содержит relative entity paths но не записывает на disk; modifies sync_orchestrator + database references к non-existent files → verify regression. Recovery state: t196 currently dirty post-test. Probable scope: path resolution / template config / entity manifest copy step. Не блокирует TASK-031 (Identity Argument для entity-level coverage applied).
