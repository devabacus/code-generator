...
npm test                # vscode-test runner (62 passing на 2026-04-26)
npm run lint            # eslint
...
5. **Проверь runtime-тесты** — executor должен был запустить `npm test` / `npm run compile` через Bash. В report.md должны быть реальные CLI-ответы (например `62 passing`). Если правка генератора/шаблона — обязательно `codegen verify --name <test_project>` с PASS и цитированными числами `errors=N, warnings=M`.
...
3. `npm test` — все тесты passing (последний baseline на 2026-04-26: **62 passing**).
