...
export class ServerpodYamlParser {
     ...
     *
     * Якорь на **колонку 0** (`^# codegen:junction:` без отступа) — это снимает
     * единственный сценарий ложного срабатывания: `#` внутри block scalar или
     * строкового default'а. Содержимое block scalar / поля обязано быть с отступом,
     * поэтому на колонке 0 его быть не может. Тот же приём применён в `parseField`
     * для quoted-строк (BUG-012 side-fix).
     *
