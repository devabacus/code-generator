...
export async function addPythonToProject(): Promise<void> {
    ...
    // Check if Python folder already exists
    if (await fileSystem.exists(pythonPath)) {
        const overwrite = await window.showWarningMessage(
            `${projectName}_python already exists. Overwrite?`,
            'Yes', 'No'
        );
        if (overwrite !== 'Yes') {
            return;
    ...
    try {
        ...
        // Run serverpod generate to create client code for python_endpoint
        await executeCommand(SERVERPOD_GENERATE, serverPath);

        window.showInformationMessage(
            `✅ Python microservice added to ${projectName}!`
        );

    } catch (error) {
        window.showErrorMessage(`Error adding Python: ${error}`);
    }
}

/**
 * Copy and adapt CI workflow for Python deployment
 */
async function copyCIWorkflow(
    ...
    const ciSourcePath = path.join(
        templatesPath, 'flutter', TEMPL_PROJECT, '.github', 'workflows', 'deployment-python.yml'
    );
    const ciDestPath = path.join(
        workspacePath, '.github', 'workflows', 'deployment-python.yml'
    );
